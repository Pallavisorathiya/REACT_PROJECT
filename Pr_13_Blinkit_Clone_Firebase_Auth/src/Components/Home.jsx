...
const Home = () => {
  ...
  return (
    <div className="home-page"> {/* Added wrapper for scoping */}
      {isLoading ? (
        <div className="spinner">
          <Spinner animation="border" variant="success" />
        </div>
      ) : isError ? (
        <div className="spinner text-danger text-center mt-5">{isError}</div>
      ) : (
        <>
          <StaticElem />
          <Categories />
          ...
          <div className="d-flex flex-wrap gap-2 my-3 px-3 align-items-center justify-content-between">
            ...
            <div className="d-flex align-items-center gap-2 ms-4">
              ...
              <Button className="short-btn" onClick={handleSorting}>
                Sort
              </Button>
            </div>
          </div>
  ...
  className="col-6 col-sm-4 col-md-3 col-lg-2"
  key={prod.id}
  onClick={() => navigate(`/category/${encodeURIComponent(prod.category)}`)}
  style={{ cursor: "pointer" }}
>
